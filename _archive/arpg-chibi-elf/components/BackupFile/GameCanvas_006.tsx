      ...
      for (const entity of renderList) {
          ...
          if (entity.type === 'player') {
              ...
              if (isPngHero) {
                  ...
                  // --- WEAPON (PNG) ---
                  if (sprites.weapon && (sprites.weapon as HTMLImageElement).naturalWidth > 0) {
                    ...
                    if (isAttacking) {
                      ...
                      swing = Math.sin(time / 300) * 0.15; // tiny idle sway
                    }

                    // Pivot point (tune once visually). Coordinates are in the 1600x2000 template space.
                    const HAND_X_BASE = heroW * 0.70; // ~1120 for 1600
                    const HAND_Y = heroH * 0.59;      // ~1180 for 2000
                    const HAND_X = facingLeft ? (heroW - HAND_X_BASE) : HAND_X_BASE;

                    // If facing left, invert swing so it feels consistent.
                    const swingSigned = facingLeft ? -swing : swing;

                    ctx.save();
                    ctx.translate(HAND_X, HAND_Y);
                    ctx.rotate(swingSigned);

                    // Tune this if the weapon looks too big/small
                    const WEAPON_SCALE = 2;
                    ctx.scale(WEAPON_SCALE, WEAPON_SCALE);

                    const w = weaponImg.width;
                    const h = weaponImg.height;

                    // Offset so the handle sits near the pivot

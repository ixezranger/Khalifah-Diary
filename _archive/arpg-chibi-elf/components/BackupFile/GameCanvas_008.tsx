...
// Tile scaling (render scale)
const TILE_SCALE = 3; 
const TILE = TILE_SIZE * TILE_SCALE;
// Ground PNG tiling density for GRASS tiles (lower => pattern looks bigger)
const GROUND_REPEAT_SCALE = 0.6;
...
const createTileTexture = (type: TileType, groundImg?: HTMLImageElement | null): HTMLCanvasElement => {
    ...
    switch(type) {
        case TileType.GRASS:
            ...
            // If a seamless ground tile image is provided, paint it first (best for custom PNG patterns).
            if (groundImg && groundImg.complete && groundImg.naturalWidth > 0) {
                // Improved: repeat the seamless PNG as a pattern so it fills the whole tile naturally
                // (no stretching artifacts), then add subtle lighting for depth/readability.
                const pattern = ctx.createPattern(groundImg, 'repeat');
                if (pattern) {
                    ctx.save();
                    ctx.scale(GROUND_REPEAT_SCALE, GROUND_REPEAT_SCALE);
                    ctx.fillStyle = pattern;
                    ctx.fillRect(
                        -radius / GROUND_REPEAT_SCALE,
                        -radius / GROUND_REPEAT_SCALE,
                        (radius * 2) / GROUND_REPEAT_SCALE,
                        (radius * 2) / GROUND_REPEAT_SCALE
                    );
                    ctx.restore();
                }

                // Soft lighting/vignette to keep character silhouettes readable on textured ground.
      ...
      if (textures) {
          for (let x = startX; x < endX; x++) {
            for (let y = startY; y < endY; y++) {
              ...
              if(screenX < -TILE || screenX > canvas.width + TILE || screenY < -TILE || screenY > canvas.height + TILE) continue;
              const img = textures[tileType];
              if (img) ctx.drawImage(img, screenX - TILE, screenY - TILE/2, TILE * 2, TILE * 2);
            }
          }
      }
      ...
      for (const entity of renderList) {
          ...
          if (entity.type === 'player') {
              ...
              if (isPngHero) {
                  ...
                  // Weapon (held in right hand) + swing animation + enhancement glow
                  if (sprites.weapon) {
                    const weaponItem = equippedItems.find(i => i.type === ItemType.WEAPON);
                    const upgradeLevel = weaponItem?.upgradeLevel || 0;

                    // Hand position in the 1600x2000 hero template (your Photoshop coords)
                    const handX = 300;
                    const handY = 1320 + bob;

                    // Pivot (grip point) in weapon-image coords (your Photoshop coords)
                    const pivotX = 470;
                    const pivotY = 830;

                    // Swing timing driven by the existing attack animation ref
                    let weaponRot = 0;
                    if (attackAnimRef.current > 0) {
                      ...
                      if (t < 0.2) {
                        ...
                        const p = (t - 0.2) / 0.6; // 0..1
                        weaponRot = -0.9 + (p * 1.6); // -0.9 -> +0.7
                      } else {
                        const p = Math.min(1, (t - 0.8) / 0.4);
                        weaponRot = 0.7 * (1 - p);
                      }
                    }

                    // Extra scale for PNG weapon (requested: x2)
                    const weaponScale = 2;

                    const flash = weaponEnhanceFlashRef.current; // seconds remaining
                    ...
                    // Base glow based on upgrade level, plus a short flash on enhancement
                    const baseGlow = Math.min(22, upgradeLevel * 2);
                    const flashGlow = 55 * flashT;
                    ctx.shadowBlur = Math.max(baseGlow, flashGlow);
                    ctx.shadowColor = flashT > 0 ? 'rgba(56, 189, 248, 0.95)' : 'rgba(34, 211, 238, 0.65)';

                    // Main draw
                    ctx.drawImage(sprites.weapon, -pivotX, -pivotY);

                    // Additive "pop" during flash (looks like a burst)
                    if (flashT > 0) {
                      ctx.save();
                      ctx.globalCompositeOperation = 'screen';
                      ctx.globalAlpha = 0.25 * flashT;
                      ctx.shadowBlur = 85 * flashT;
                  ...
                  // Right arm last so the hand sits on top of the weapon handle

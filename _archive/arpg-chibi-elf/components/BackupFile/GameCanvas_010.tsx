      ...
      for (const entity of renderList) {
          ...
          if (entity.type === 'player') {
              ...
              if (isPngHero) {
                  ...
                  // Arms + Weapon (order depends on facing so the weapon is held by the front hand)
                  if (facingLeft) {
                    // Back arm first
                  ...
                  // Weapon (attached to the active hand) + swing animation + enhancement glow
                  if (sprites.weapon) {
                    ...
                    const handX = facingLeft ? leftHandX : rightHandX;
                    ...
                    // Natural "holding" angle (degrees ≈ -20)
                    const BASE_HOLD_ROT = -0.35;
                    const facingSign = facingLeft ? -1 : 1;

                    // Glow: keep subtle (also avoids duplicate/ghost weapon artifacts)
                    const baseGlow = Math.min(18, upgradeLevel * 1.6);
                    const flashGlow = 35 * flashT;
                    const glow = Math.max(baseGlow, flashGlow);

                    ctx.save();
                    ctx.translate(handX, handY);
                    ctx.rotate((BASE_HOLD_ROT * facingSign) + weaponRot);
                    ctx.scale(weaponScale, weaponScale);

                    ctx.shadowBlur = glow;
                    ctx.shadowColor = flashT > 0 ? 'rgba(255,255,255,0.75)' : 'rgba(0,0,0,0.35)';

                    // Single draw only (removes the small "ghost sword" issue)
                    ctx.drawImage(sprites.weapon, -pivotX, -pivotY);

                    // Reset shadow to not affect later layers
                    ctx.shadowBlur = 0;
                  ...
                  // Front arm last so the hand sits on top of the weapon handle
                  if (facingLeft) {
                    if (sprites.armL) ctx.drawImage(sprites.armL, 0, bob);
                  } else {
                    if (sprites.armR) ctx.drawImage(sprites.armR, 0, bob);
